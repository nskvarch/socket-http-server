...
def response_not_found():
    ...
    return b"\r\n".join([
        ...
        b"",
        b"What you are looking for does not seem to exist!"
    ])
...
def response_path(path):
    ...
    req_path = os.path.abspath(".") + "/webroot" + path

    if os.path.isdir(req_path):
        str_conv = " \n".join(os.listdir(req_path))
        content = str_conv.encode()
        mime_type = b"text/plain"
    elif os.path.isfile(req_path):
        content = open(req_path, "rb").read()
        mime_type = mimetypes.guess_type(req_path)[0].encode()
    else:
        content = b"NotImplamented"
        mime_type = b"NotImplamented"
        raise NameError
